...
A single local CLI for checking the status of multiple AI coding agents (Claude Code, and other providers as adapters are added) running on your machine — session state, current task, context usage, cost, and rate limits, regardless of how each agent reports its own status.
...
Session state is stored as flat JSON files under `$XDG_STATE_HOME/agent-status-collector` (defaults to `~/.local/state/agent-status-collector`). Pass `--debug` (or persist it via `$XDG_CONFIG_HOME/agent-status-collector/config.json`) to retain raw hook/statusline payloads for troubleshooting.
