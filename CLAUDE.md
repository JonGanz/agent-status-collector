...
- **Hooks** (`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Notification`, `Stop`, `SubagentStop`, `PreCompact`, `SessionEnd`) drive `State` transitions — see the switch in `claudecode.go`'s `handleLifecycleHook`. `Stop` (turn concluded) normally sets `StateDone`, distinct from `SessionEnd` (session terminated, `StateStopped`) — except when something the turn triggered hasn't actually resolved yet: an unapproved `ExitPlanMode` (`awaiting_plan_approval` in `Extra`) keeps it `StateBlocked`, and one or more still-running Task-tool subagents (`pending_subagents` counter in `Extra`, incremented on `PreToolUse` for `tool_name: Task`, decremented on `SubagentStop`) keep it `StateActive` — a subagent finishing doesn't mean the parent is done, and a subagent launched to run in the background can still be working well after the parent's own `Stop` fires. `Notification` is the only "needs attention" signal Claude Code exposes, but it fires for two unrelated reasons distinguishable only by free-text message content — `isPermissionRequest` does a best-effort substring match (`"permission"`) to set `StateBlocked` for genuine tool-permission requests, versus `StateDone` for the ~60s idle-waiting reminder that follows an already-completed `Stop`. This heuristic is inherently a little fragile (Claude Code doesn't document the message wording as a stable API) but it's the only signal available — don't collapse the two back into one state, that was the whole point of splitting them out.
